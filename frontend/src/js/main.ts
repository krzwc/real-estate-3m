...
mapboxgl.accessToken = 'F6GYQqu0llmxsuwEKDsi';
...
class App {
    ...
    constructor() {
        this.map = new mapboxgl.Map({
            ...
            style:
                'https://api.maptiler.com/maps/dbbf53d1-09f9-456b-bd9b-c354829441ed/style.json?key=F6GYQqu0llmxsuwEKDsi',
            ...
            compose(
                (locations) => {
                    this.searchInput.addEventListener(
                        'keyup',
                        ({ target: { value: inputValue } }: HTMLElementEvent<HTMLInputElement>) => {
